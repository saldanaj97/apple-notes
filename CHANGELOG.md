...
- **Improved 'New Note' Functionality**: 'New Note' command has been updated to take in a title in the search bar which then goes on to a 'NewNoteForm' view that allows the user to paste in text with a view of the text in its intended format rather than one line of text.
- **Added New 'New Note from Clipboard' Command**: The 'New Note from Clipboard' command will now ask for a title in the search bar and will then create a note with the data from the clipboard while maintaining format.
